...
class AttentionWeightComputation(Function):
    ...
    def forward(
        ...
        output = torch.zeros(
            (total_query_num, local_size, nhead), device=query_batch_cnt.device, dtype=torch.float32, requires_grad=True
        )
    ...
    def backward(ctx, grad_out: torch.Tensor):
        ...
        grad_query_features = torch.zeros(
            (total_query_num, nhead, hdim), dtype=torch.float32, requires_grad=True, device=grad_out.device
        )
        # grad_key_features = Variable(torch.cuda.FloatTensor(total_key_num, nhead, hdim).zero_())
        grad_key_features = torch.zeros(
            (total_key_num, nhead, hdim), requires_grad=True, device=grad_out.device, dtype=torch.float32
        )
        ...
        attention_cuda.attention_weight_computation_grad_wrapper_v2(
            ...
            key_features,
            grad_out_data,
            grad_query_features.data,
            grad_key_features.data,
        )
...
class AttentionValueComputation(Function):
    ...
    def forward(
        ...
        output = torch.zeros((total_query_num, nhead, hdim), device=query_batch_cnt.device, dtype=torch.float32)
    ...
    def backward(ctx, grad_out: torch.Tensor):
        ...
        # grad_attn_weight = Variable(torch.cuda.FloatTensor(total_query_num, local_size, nhead).zero_())
        grad_attn_weight = torch.zeros(
            (total_query_num, local_size, nhead), dtype=torch.float32, requires_grad=True, device=grad_out.device
        )
        # grad_value_features = Variable(torch.cuda.FloatTensor(total_key_num, nhead, hdim).zero_())
        grad_value_features = torch.zeros(
            (total_key_num, nhead, hdim), dtype=torch.float32, requires_grad=True, device=grad_out.device
        )
